...
import UIKit
// New protocol for load more items
public protocol MSGMessengerLoadMoreItems : NSObjectProtocol {
    func loadMoreItems(callCount:Int)
}
extension MSGMessengerLoadMoreItems {
    public func loadMoreItems(callCount:Int) {}
}
extension MSGMessengerViewController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UICollectionViewDataSourcePrefetching, MSGSectionReusableViewDelegate {
    ...
    // MARK: - ScroolViewDidScroll
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if scrollView.contentOffset.y < 0 && self.collectionView.isLoadingMoreItemsCount == 0 {
            if self.collectionView.isLoadingMoreItemsCount == 0 {
                self.collectionView.isScrollEnabled = false
                self.collectionView.isLoadingMoreItemsCount += 1
                delegateMoreItems?.loadMoreItems(callCount: self.collectionView.isLoadingMoreItemsCount)
            }
        }
    }
